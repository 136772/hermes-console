...
已发布的功能都是稳定可用的（在飞牛 NAS 上长期实战）；下面这些是**尚未实现**的候选清单，
顺序不代表优先级——想要哪个优先做，去 [Issues](https://github.com/136772/hermes-console/issues) 说一声。

| 方向 | 候选功能 | 现状痛点 |
|---|---|---|
| **对话** | 气泡内 Markdown 渲染 + 代码高亮；会话导出（MD / JSON） | 回复目前是纯文本 |
| **智能体** | 团队定时运行（cron 触发）、运行历史持久化与回放、流水线人工审批节点 | 编排只能手动点一次，运行记录不落盘 |
| **成本** | 按智能体 / 按会话归集成本；月末趋势预测 | 目前只按模型归集 |
| **监控** | 趋势历史跨重启持久化（SQLite） | 60 分钟趋势图在工作台重启后清零 |
| **安全** | 密码之上叠加 TOTP 两步验证 | 目前只有密码 + 失败锁定 |
| **编辑器** | 工作区跨文件搜索替换 | 目前只能单文件改 |
| **界面** | 页面中英双语切换（i18n） | 目前 UI 文案只有中文，仅文档是双语的 |
| **规模** | 一个控制台监控多个 Hermes 实例 / 数据目录 | 目前一台控制台对应一个 `HERMES_DIR` |
